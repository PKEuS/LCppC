...
int main()
{
    ...
    fout << "# To compile as debug build use 'make DEBUG=yes'\n";
    ...
    // pthread
    fout << "LDFLAGS+=-pthread\n";

    // The _GLIBCXX_DEBUG doesn't work in cygwin or other Win32 systems.
    fout << "# Set the CPPCHK_GLIBCXX_DEBUG flag. This flag is not used in release builds.\n"
    ...
    fout <<
         ...
         "else\n"
         "    override CXXFLAGS += -march=native -mtune=native -O2 -DNDEBUG\n"
         "endif\n\n";

    // Rules
    fout << "ifeq ($(HAVE_RULES),yes)\n"
         << "    override CXXFLAGS += -DHAVE_RULES $(shell pcre-config --cflags)\n"
         << "    ifdef LIBS\n"
    ...
    makeConditionalVariable(fout, "INCLUDE_FOR_LIB", "-Ilib -isystem externals -isystem externals/simplecpp -isystem externals/picojson -isystem externals/tinyxml2");
