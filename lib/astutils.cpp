...
bool isUniqueExpression(const Token* tok)
{
    ...
    if (tok->function()) {
        ...
        bool other = std::any_of(scope->varlist.cbegin(), scope->varlist.cend(), [=](const Variable& v) {
            if (varType)
        ...
        if (other)
            return false;
        if (fun) {
            other = std::any_of(fun->argumentList.cbegin(), fun->argumentList.cend(), [=](const Variable& v) {
                if (varType)
                    return v.type() && v.type()->name() == varType->name() && v.name() != var->name();
                return v.isFloatingType() == var->isFloatingType() &&
                       v.isEnumType() == var->isEnumType() &&
...
const Variable* getArgumentVar(const Token* tok, unsigned int argnr)
{
    if (!tok)
        return nullptr;
    if (tok->function())
