        ...
        for (const Token *tok = function->bodyStart; tok != function->bodyEnd; tok = tok->next()) {
            ...
            for (const ValueFlow::Value &value : tok->values()) {
                ...
                if (value.isImpossible())
                    continue;
                if (value.isInconclusive() && !mCtx.project->certainty.isEnabled(Certainty::inconclusive))
                    continue;
                ...
                if ((!container->arrayLike_indexOp && !container->stdStringLike) || !parent)
                    continue;
...
void CheckStl::outOfBoundsError(const Token *tok, const std::string &containerName, const ValueFlow::Value *containerSize, const std::string &index, const ValueFlow::Value *indexValue)
{
    ...
    reportError(errorPath,
                ...
                CWE398,
                (containerSize && containerSize->isInconclusive()) || (indexValue && indexValue->isInconclusive()) ? Certainty::inconclusive : Certainty::safe);
}
...
bool CheckStl::isContainerSize(const Token *containerToken, const Token *expr) const
{
    ...
    if (!Token::Match(expr->astOperand1(), ". %name% ("))
        return false;
    if (!isSameExpression(mCtx.tokenizer->isCPP(), false, containerToken, expr->astOperand1()->astOperand1(), mCtx.project->library, false, false))
        return false;
...
bool CheckStl::isContainerSizeGE(const Token * containerToken, const Token *expr) const
{
    ...
    if (expr->str() == "+") {
        ...
        if (isContainerSize(containerToken, expr->astOperand1()))
            ...
            return false;
        return op && op->getValueGE(0, mCtx.project);
    }
    return false;
}
...
void CheckStl::iteratorsError(const Token* tok, const std::string& containerName1, const std::string& containerName2)
{
    reportError(tok, Severity::error, "iterators1",
                ...
                "Same iterator is used with different containers '" + containerName1 + "' and '" + containerName2 + "'.", CWE664, Certainty::safe);
}
...
void CheckStl::dereferenceErasedError(const Token *erased, const Token* deref, const std::string &itername, bool inconclusive)
{
    if (erased) {
        ...
        reportError(callstack, Severity::error, "eraseDereference",
                    ...
                    "Dereferencing or comparing it with another iterator is invalid operation.", CWE664, inconclusive ? Certainty::inconclusive : Certainty::safe);
    } else {
...
void CheckStl::iterators()
{
    for (const Variable* var : mCtx.symbolDB->variableList()) {
        ...
        if (!isIterator(var, inconclusiveType))
            continue;
        if (inconclusiveType && !mCtx.project->certainty.isEnabled(Certainty::inconclusive))
            continue;
...
void CheckStl::mismatchingContainerExpressionError(const Token *tok1, const Token *tok2)
{
    ...
    reportError(tok1, Severity::warning, "mismatchingContainerExpression",
                ...
                expr1 + "' and '" + expr2 + "' are used together.", CWE664, Certainty::safe);
}

void CheckStl::sameIteratorExpressionError(const Token *tok)
{
    reportError(tok, Severity::style, "sameIteratorExpression", "Same iterators expression are used for algorithm.", CWE664, Certainty::safe);
}
...
bool CheckStl::checkIteratorPair(const Token* tok1, const Token* tok2)
{
    ...
    if (val1.tokvalue && val2.tokvalue && val1.lifetimeKind == val2.lifetimeKind) {
        ...
        if (isSameExpression(true, false, val1.tokvalue, val2.tokvalue, mCtx.project->library, false, false))
            return false;
    ...
    if (iter1 && iter2 && !isSameExpression(true, false, iter1, iter2, mCtx.project->library, false, false)) {
    ...
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            for (std::size_t argnr = 1; argnr <= args.size(); ++argnr) {
                const Library::ArgumentChecks::IteratorInfo *i = mCtx.project->library.getArgIteratorInfo(ftok, argnr);
                if (!i)
                    continue;
                    ...
                    for (ArgIteratorInfo iter1 : cargs) {
                        for (ArgIteratorInfo iter2 : cargs) {
                            ...
                            if (iter1.info->first && iter2.info->last &&
                                isSameExpression(true, false, iter1.tok, iter2.tok, mCtx.project->library, false, false))
    ...
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            for (const LifetimeToken& lt:getLifetimeTokens(tok)) {
                ...
                if (isSameExpression(true, false, contTok, val.tokvalue, mCtx.project->library, false, false))
                    continue;
...
{
    const Library& library = mCtx.project->library;
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (tok->varId() == 0)
                continue;
    ...
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            for (const Token* tok2 = blockStart; tok2 != blockEnd; tok2 = tok2->next()) {
                ...
                if (!s)
                    continue;
                if (isReturnScope(s->bodyEnd, &mCtx.project->library))
                    continue;
...
{
    if (at)
        reportError(tok, Severity::error, "stlOutOfBounds", "$symbol:" + var + "\nWhen " + num + "==$symbol.size(), $symbol.at(" + num + ") is out of bounds.", CWE788, Certainty::safe);
    else
        reportError(tok, Severity::error, "stlOutOfBounds", "$symbol:" + var + "\nWhen " + num + "==$symbol.size(), $symbol[" + num + "] is out of bounds.", CWE788, Certainty::safe);
}
    ...
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (!var || tok == var->nameToken())
                continue;
            const Library::Container * const container = mCtx.project->library.detectContainer(var->typeStartToken());
            if (!container || !container->arrayLike_indexOp)
                continue;
            const ValueFlow::Value *index = tok->next()->astOperand2()->getValueLE(-1, mCtx.project);
            if (!index)
                continue;
...
void CheckStl::negativeIndexError(const Token *tok, const ValueFlow::Value &index)
{
    ...
    reportError(errorPath, index.errorSeverity() ? Severity::error : Severity::warning, "negativeContainerIndex", errmsg.str(), CWE786, index.isInconclusive() ? Certainty::inconclusive : Certainty::safe);
}
...
void CheckStl::stlBoundaries()
{
    for (const Variable* var : mCtx.symbolDB->variableList()) {
        ...
        const Library::Container* container = mCtx.project->library.detectContainer(var->typeStartToken(), true);
...
void CheckStl::stlBoundariesError(const Token *tok)
{
    reportError(tok, Severity::error, "stlBoundaries",
                ...
                "container is not guaranteed. One should use operator!= instead to compare iterators.", CWE664, Certainty::safe);
}
...
void CheckStl::if_find()
{
    ...
    const bool printPerformance = mCtx.project->severity.isEnabled(Severity::performance);
    ...
    for (const Scope &scope : mCtx.symbolDB->scopeList) {
        ...
        for (const Token *tok = conditionStart; tok->str() != "{"; tok = tok->next()) {
            ...
            if (Token::Match(tok, "%name% ("))
                ...
                container = mCtx.project->library.detectContainer(tok->variable()->typeStartToken());
                funcTok = tok->tokAt(2);
            }
            ...
            // check also for vector-like or pointer containers
                ...
                if (tok->variable()->isArrayOrPointer())
                    ...
                    container = mCtx.project->library.detectContainer(tok->variable()->typeStartToken()); // outer container
                    ...
                    if (container && container->type_templateArgNo >= 0 && tok2) {
                        ...
                        container = mCtx.project->library.detectContainer(tok2); // innner container
                    } else
                        container = nullptr;
                }
            }
...
{
    if (str && mCtx.project->standards.cpp >= Standards::CPP20)
        reportError(tok, Severity::performance, "stlIfStrFind",
                    ...
                    "you should compare with std::string::npos.", CWE597, Certainty::safe);
    if (!str)
        reportError(tok, Severity::warning, "stlIfFind", "Suspicious condition. The result of find() is an iterator, but it is not properly checked.", CWE398, Certainty::safe);
}
...
void CheckStl::checkFindInsert()
{
    ...
    for (const Scope *scope : mCtx.symbolDB->functionScopes) {
        for (const Token *tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            const Token *valueTok = findInsertValue(thenTok, containerTok, keyTok, mCtx.project->library);
            if (!valueTok)
                continue;

            if (Token::simpleMatch(thenTok->link(), "} else {")) {
                const Token *valueTok2 =
                    findInsertValue(thenTok->link()->tokAt(2), containerTok, keyTok, mCtx.project->library);
                if (!valueTok2)
                    continue;
                if (isSameExpression(true, true, valueTok, valueTok2, mCtx.project->library, true, true)) {
...
{
    reportError(
        tok, Severity::performance, "stlFindInsert", "Searching before insertion is not necessary.", CWE398, Certainty::safe);
}
...
void CheckStl::size()
{
    ...
    if (mCtx.project->standards.cpp >= Standards::CPP11)
        return;
...
void CheckStl::sizeError(const Token *tok)
{
    ...
    reportError(tok, Severity::performance, "stlSize",
                ...
                "guaranteed to take constant time.", CWE398, Certainty::safe);
}
...
void CheckStl::redundantIfRemoveError(const Token *tok)
{
    reportError(tok, Severity::style, "redundantIfRemove",
                ...
                "Redundant checking of STL container element existence before removing it. "
                "It is safe to call the remove method on a non-existing element.", CWE398, Certainty::safe);
}
...
void CheckStl::missingComparisonError(const Token *incrementToken1, const Token *incrementToken2)
{
    ...
    reportError(callstack, Severity::warning, "StlMissingComparison", errmsg.str(), CWE834, Certainty::safe);
}
    ...
    for (const Scope &scope : mCtx.symbolDB->scopeList) {
        ...
        for (const Token *tok = scope.bodyStart; tok && tok != scope.bodyEnd; tok = tok->next()) {
            ...
            if (Token::Match(tok, "throw %var% . c_str|data ( ) ;") && isLocal(tok->next()) &&
                ...
                if (Token::Match(op21->tokAt(-5), "%var% . str ( )")) { // a = a.str().c_str();
...
void CheckStl::string_c_strError(const Token* tok)
{
    reportError(tok, Severity::error, "stlcstr", "Dangerous usage of c_str(). The value returned by c_str() is invalid after this call.\n"
                "Dangerous usage of c_str(). The c_str() return value is only valid until its string is deleted.", CWE664, Certainty::safe);
}
...
void CheckStl::string_c_strParam(const Token* tok, unsigned int number)
{
    ...
    reportError(tok, Severity::performance, "stlcstrParam", oss.str(), CWE704, Certainty::safe);
}
...
void CheckStl::uselessCallsReturnValueError(const Token *tok, const std::string &varname, const std::string &function)
{
    ...
    reportError(tok, Severity::warning, "uselessCallsCompare", errmsg.str(), CWE628, Certainty::safe);
}
...
void CheckStl::uselessCallsSwapError(const Token *tok, const std::string &varname)
{
    reportError(tok, Severity::performance, "uselessCallsSwap",
                ...
                "code is inefficient. Is the object or the parameter wrong here?", CWE628, Certainty::safe);
}

void CheckStl::uselessCallsSubstrError(const Token *tok, bool empty)
{
    if (empty)
        reportError(tok, Severity::performance, "uselessCallsSubstr", "Ineffective call of function 'substr' because it returns an empty string.", CWE398, Certainty::safe);
    else
        reportError(tok, Severity::performance, "uselessCallsSubstr", "Ineffective call of function 'substr' because it returns a copy of the object. Use operator= instead.", CWE398, Certainty::safe);
}

void CheckStl::uselessCallsEmptyError(const Token *tok)
{
    reportError(tok, Severity::warning, "uselessCallsEmpty", "Ineffective call of function 'empty()'. Did you intend to call 'clear()' instead?", CWE398, Certainty::safe);
}
...
void CheckStl::uselessCallsRemoveError(const Token *tok, const std::string& function)
{
    reportError(tok, Severity::warning, "uselessCallsRemove",
                ...
                "Elements past new end remain valid but with unspecified values. Use the erase method of the container to delete them.", CWE762, Certainty::safe);
}
...
void CheckStl::checkDereferenceInvalidIterator2()
{
    ...
    for (const Token *tok = mCtx.tokenizer->tokens(); tok; tok = tok->next()) {
        ...
        for (const ValueFlow::Value& value:tok->values()) {
            ...
            bool unknown = false;
            if (!CheckNullPointer::isPointerDeRef(tok, unknown, mCtx.project)) {
                if (!unknown)
                    continue;
                inconclusive = true;
            }
...
void CheckStl::dereferenceInvalidIteratorError(const Token* tok, const ValueFlow::Value *value, bool inconclusive)
{
    ...
    if (!tok || !value) {
        reportError(tok, Severity::error, "derefInvalidIterator", "Dereference of an invalid iterator", CWE825, Certainty::safe);
        reportError(tok, Severity::warning, "derefInvalidIteratorRedundantCheck", errmsgcond, CWE825, Certainty::safe);
        return;
    }
    ...
    if (value->condition) {
        reportError(errorPath, Severity::warning, "derefInvalidIteratorRedundantCheck", errmsgcond, CWE825, (inconclusive || value->isInconclusive()) ? Certainty::inconclusive : Certainty::safe);
    } else {
        std::string errmsg;
        ...
        reportError(errorPath,
                    ...
                    errmsg,
                    CWE825, (inconclusive || value->isInconclusive()) ? Certainty::inconclusive : Certainty::safe);
    }
}
...
void CheckStl::dereferenceInvalidIteratorError(const Token* deref, const std::string &iterName)
{
    reportError(deref, Severity::warning,
                ...
                "Possible dereference of an invalid iterator: $symbol. Make sure to check that the iterator is valid before dereferencing it - not after.", CWE825, Certainty::safe);
}
...
void CheckStl::useStlAlgorithmError(const Token *tok, const std::string &algoName)
{
    reportError(tok, Severity::style, "useStlAlgorithm",
                "Consider using " + algoName + " algorithm instead of a raw loop.", CWE398, Certainty::safe);
}
        ...
        for (const Token *tok = function->bodyStart; tok != function->bodyEnd; tok = tok->next()) {
            ...
            if (tok->str() == "for") {
                ...
                for (int argnr = 1; argnr <= args.size(); ++argnr) {
                    const Library::ArgumentChecks::IteratorInfo *i = mCtx.project->library.getArgIteratorInfo(tok, argnr);
                    if (!i)
                        continue;
                    const Token * const argTok = args[argnr - 1];
...
void CheckStl::globalLockGuardError(const Token* tok)
{
    reportError(tok, Severity::warning,
                "globalLockGuard",
                "Lock guard is defined globally. Lock guards are intended to be local. A global lock guard could lead to a deadlock since it won't unlock until the end of the program.", CWE833, Certainty::safe);
}
...
void CheckStl::localMutexError(const Token* tok)
{
    reportError(tok, Severity::warning,
                "localMutex",
                "The lock is ineffective because the mutex is locked at the same scope as the mutex itself.", CWE667, Certainty::safe);
}

void CheckStl::checkMutexes()
{
    for (const Scope *function : mCtx.symbolDB->functionScopes) {
        std::set<unsigned int> checkedVars;
        for (const Token *tok = function->bodyStart; tok != function->bodyEnd; tok = tok->next()) {
