...
struct ReverseTraversal {
    ReverseTraversal(const ValuePtr<Analyzer>& analyzer, const Project* project)
                    ...
                    if (rhsAction.isRead()) {
                        ...
                        if (a) {
                            valueFlowGenericForward(nextAfterAstRightmostLeaf(assignTok->astOperand2()),
                                                    ...
                                                    a,
                                                    project);
                            valueFlowGenericReverse(assignTok->astOperand1()->previous(), a, project);
                        }
                    }
                }
                if (!continueB)
                    break;
                valueFlowGenericForward(assignTop->astOperand2(), analyzer, project);
            ...
            if (tok->str() == "}") {
                ...
                if (inLoop) {
                    if (condAction.isModified())
                        break;
                    valueFlowGenericForward(condTok, analyzer, project);
                }
...
void valueFlowGenericReverse(Token* start, const ValuePtr<Analyzer>& a, const Project* project)
{
    ReverseTraversal rt{a, project };
    rt.traverse(start);
}
