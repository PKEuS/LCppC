...
private:
    ...
    void destructorsError(const Token * const tok, const std::string &className) {
        reportError(tok, Severity::warning, "exceptThrowInDestructor",
                    ...
                    "is thrown that is caught in an outer scope the program will terminate.", CWE398, Certainty::safe);
    }

    void deallocThrowError(const Token * const tok, const std::string &varname) {
        reportError(tok, Severity::warning, "exceptDeallocThrow", "Exception thrown in invalid state, '" +
                    varname + "' points at deallocated memory.", CWE398, Certainty::safe);
    }
    ...
    void rethrowCopyError(const Token * const tok, const std::string &varname) {
        reportError(tok, Severity::style, "exceptRethrowCopy",
                    ...
                    "To rethrow the caught exception without unnecessary copying or slicing, use a bare 'throw;'.", CWE398, Certainty::safe);
    }
    ...
    void catchExceptionByValueError(const Token *tok) {
        reportError(tok, Severity::style,
                    ...
                    "as a (const) reference which is usually recommended in C++.", CWE398, Certainty::safe);
    }

    void noexceptThrowError(const Token * const tok) {
        reportError(tok, Severity::error, "throwInNoexceptFunction", "Exception thrown in function declared not to throw exceptions.", CWE398, Certainty::safe);
    }
