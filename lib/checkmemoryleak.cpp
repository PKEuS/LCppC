...
CheckMemoryLeak::AllocType CheckMemoryLeak::getAllocationType(const Token *tok2, unsigned int varid, std::vector<const Function*> *callstack) const
{
    ...
    if (!Token::Match(tok2, "%name% ::|. %type%")) {
        ...
        if (mCtx_.tokenizer->isCPP() && tok2->str() == "new") {
        ...
        if (mCtx_.project->hasLibrary("posix")) {
        ...
        const int alloctype = mCtx_.project->library.getAllocId(tok2, -1);
        if (alloctype > 0) {
            if (alloctype == mCtx_.project->library.deallocId("free"))
                return Malloc;
            if (alloctype == mCtx_.project->library.deallocId("fclose"))
                return File;
...
CheckMemoryLeak::AllocType CheckMemoryLeak::getReallocationType(const Token *tok2, unsigned int varid) const
{
    ...
    const Library::AllocFunc *f = mCtx_.project->library.getReallocFuncInfo(tok2);
    ...
    const int realloctype = mCtx_.project->library.getReallocId(tok2, -1);
    if (realloctype > 0) {
        if (realloctype == mCtx_.project->library.deallocId("free"))
            return Malloc;
        if (realloctype == mCtx_.project->library.deallocId("fclose"))
            return File;
...
{
    if (mCtx_.tokenizer->isCPP() && tok->str() == "delete" && tok->astOperand1()) {
    ...
    if (Token::Match(tok, "%name% (")) {
        ...
        for (const Token* tok2 = tok->tokAt(2); tok2; tok2 = tok2->nextArgument()) {
            ...
            if (Token::Match(vartok, "%varid% )|,|-", varid)) {
                ...
                const int dealloctype = mCtx_.project->library.getDeallocId(tok, argNr);
                if (dealloctype > 0) {
                    if (dealloctype == mCtx_.project->library.deallocId("free"))
                        return Malloc;
                    if (dealloctype == mCtx_.project->library.deallocId("fclose"))
                        return File;
...
bool CheckMemoryLeak::isReopenStandardStream(const Token *tok) const
{
    if (getReallocationType(tok, 0) == File) {
        const Library::AllocFunc *f = mCtx_.project->library.getReallocFuncInfo(tok);
...
{
    const ErrorMessage errmsg(callstack, mCtx_.tokenizer ? &mCtx_.tokenizer->list : nullptr, severity, id, msg, Certainty::safe, cwe);
    if (mCtx_.errorLogger)
        mCtx_.errorLogger->reportErr(errmsg);
    else
...
CheckMemoryLeak::AllocType CheckMemoryLeak::functionReturnType(const Function* func, std::vector<const Function*> *callstack) const
{
    ...
    for (const Token* tok = func->functionScope->bodyStart; tok != func->functionScope->bodyEnd; tok = tok->next()) {
        ...
        if (!mCtx_.tokenizer->isC() && Token::Match(tok, "[(,] %varid% [,)]", varid)) {
            return No;
        }
        if (Token::Match(tok, "[(,] & %varid% [.,)]", varid)) {
            return No;
...
const char *CheckMemoryLeak::functionArgAlloc(const Function *func, unsigned int targetpar, AllocType &allocType) const
{
    ...
    std::vector<Variable>::const_iterator arg = func->argumentList.begin();
    ...
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        ...
        for (const Token *tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (tok->varId() > 0 && Token::Match(tok, "%name% =")) {
                ...
                if (!reallocTok)
                    continue;
                const Library::AllocFunc* f = mCtx.project->library.getReallocFuncInfo(reallocTok);
                if (!(f && f->arg == -1 && mCtx.project->library.isnotnoreturn(reallocTok)))
                    continue;
                ...
                if (Token::simpleMatch(tokEndRealloc->next(), "; if (") &&
                    ...
                    if (tokEndBrace && mCtx.tokenizer->isScopeNoReturn(tokEndBrace))
                        continue;
                }
...
void CheckMemoryLeakInClass::check()
{
    ...
    for (const Scope * scope : mCtx.symbolDB->classAndStructScopes) {
...
void CheckMemoryLeakInClass::unsafeClassError(const Token *tok, const std::string &classname, const std::string &varname)
{
    ...
    reportError(tok, Severity::style, "unsafeClassCanLeak",
                ...
                "The class '" + classname + "' is unsafe, wrong usage can cause memory/resource leaks for '" + varname + "'. This can for instance be fixed by adding proper cleanup in the destructor.", CWE398, Certainty::safe);
}
...
{
    reportError(tok, Severity::warning, "publicAllocationError", "$symbol:" + varname + "\nPossible leak in public function. The pointer '$symbol' is not deallocated before it is allocated.", CWE398, Certainty::safe);
}
...
void CheckMemoryLeakStructMember::checkStructVariable(const Variable * const variable)
{
    ...
    if (variable->isPointer()) {
        ...
        if (!isMalloc(variable))
            return;
    } else if (!mCtx.tokenizer->isC() && (!variable->typeScope() || variable->typeScope()->getDestructor())) {
...
void CheckMemoryLeakNoVar::checkForUnreleasedInputArgument(const Scope *scope)
{
    ...
    for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
        ...
        if ((mCtx.tokenizer->isCPP() && functionName == "delete") ||
            functionName == "free" ||
            functionName == "fclose" ||
...
void CheckMemoryLeakNoVar::checkForUnsafeArgAlloc(const Scope *scope)
{
    ...
    if (!mCtx.tokenizer->isCPP() || !mCtx.project->certainty.isEnabled(Certainty::inconclusive) || !mCtx.project->severity.isEnabled(Severity::warning))
        return;
    ...
    for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
        if (Token::Match(tok, "%name% (")) {
            ...
            for (const Token *tok2 = tok->tokAt(2); tok2 && tok2 != endParamToken; tok2 = tok2->nextArgument()) {
                ...
                if (Token::Match(tok2, "shared_ptr|unique_ptr @< ( new %name%")) {
                    pointerType = tok2;
                } else if (!isNothrow) {
                    if (Token::Match(tok2, "%name% ("))
                        functionCalled = tok2;
...
{
    reportError(loc, Severity::error, "leakNoVarFunctionCall", "Allocation with " + alloc + ", " + functionCall + " doesn't release it.", CWE772, Certainty::safe);
}

void CheckMemoryLeakNoVar::returnValueNotUsedError(const Token *tok, const std::string &alloc)
{
    reportError(tok, Severity::error, "leakReturnValNotUsed", "$symbol:" + alloc + "\nReturn value of allocation function '$symbol' is not stored.", CWE771, Certainty::safe);
}
