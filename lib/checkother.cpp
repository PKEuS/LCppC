...
void CheckOther::checkCastIntToCharAndBack()
{
    ...
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            } else if (mCtx.tokenizer->isCPP() && (Token::Match(tok, "EOF %comp% ( %var% = std :: cin . get (") || Token::Match(tok, "EOF %comp% ( %var% = cin . get ("))) {
                tok = tok->tokAt(3);
                const Variable *var = tok->variable();
                if (var && var->typeEndToken()->str() == "char" && !var->typeEndToken()->isSigned()) {
                    checkCastIntToCharAndBackError(tok, "cin.get");
                }
            } else if (mCtx.tokenizer->isCPP() && (Token::Match(tok, "%var% = std :: cin . get (") || Token::Match(tok, "%var% = cin . get ("))) {
                const Variable *var = tok->variable();
                if (var && var->typeEndToken()->str() == "char" && !var->typeEndToken()->isSigned()) {
...
void CheckOther::checkCastIntToCharAndBackError(const Token *tok, const std::string &strFunctionName)
{
    reportError(
        ...
        "when the file contains a matching character.", CWE197, Certainty::safe
    );
}
...
void CheckOther::clarifyCalculationError(const Token *tok, const std::string &op)
{
    ...
    reportError(tok,
                ...
                "The code '" + calc + "' should be written as either '" + s1 + "' or '" + s2 + "'.", CWE783, Certainty::safe);
}
...
void CheckOther::clarifyStatementError(const Token *tok)
{
    reportError(tok, Severity::warning, "clarifyStatement", "In expression like '*A++' the result of '*' is unused. Did you intend to write '(*A)++;'?\n"
                ...
                "Thus, the dereference is meaningless. Did you intend to write '(*A)++;'?", CWE783, Certainty::safe);
}
...
{
    if (!mCtx.project->certainty.isEnabled(Certainty::inconclusive) || !mCtx.project->severity.isEnabled(Severity::warning))
        return;
...
void CheckOther::warningOldStylePointerCast()
{
    ...
    if (!mCtx.project->severity.isEnabled(Severity::style) || !mCtx.tokenizer->isCPP())
        return;
...
void CheckOther::cstyleCastError(const Token *tok)
{
    reportError(tok, Severity::style, "cstyleCast",
                ...
                "which kind of cast is expected. See also: https://www.securecoding.cert.org/confluence/display/cplusplus/EXP05-CPP.+Do+not+use+C-style+casts.", CWE398, Certainty::safe);
}
...
void CheckOther::invalidPointerCastError(const Token* tok, const std::string& from, const std::string& to, bool inconclusive, bool toIsInt)
{
    if (toIsInt) { // If we cast something to int*, this can be useful to play with its binary data representation
        reportError(tok, Severity::portability, "invalidPointerCast", "Casting from " + from + " to " + to + " is not portable due to different binary data representations on different platforms.", CWE704, inconclusive ? Certainty::inconclusive : Certainty::safe);
    } else
        reportError(tok, Severity::portability, "invalidPointerCast", "Casting between " + from + " and " + to + " which have an incompatible binary data representation.", CWE704, Certainty::safe);
}
    ...
    for (const Scope *scope : mCtx.symbolDB->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if ((tok->isAssignmentOp() || tok->tokType() == Token::eIncDecOp) && tok->astOperand1()) {
                ...
                if (mCtx.tokenizer->isCPP() && tok->astOperand1()->valueType() && tok->astOperand1()->valueType()->typeScope) {
                ...
                if (inconclusive && !mCtx.project->certainty.isEnabled(Certainty::inconclusive))
                    continue;
...
void CheckOther::redundantAssignmentError(const Token *tok1, const Token* tok2, const std::string& var, bool inconclusive)
{
    ...
    if (inconclusive)
        ...
        reportError(errorPath, Severity::style, "redundantAssignment",
                    "$symbol:" + var + "\n"
                    "Variable '$symbol' is reassigned a value before the old one has been used.", CWE563, Certainty::safe);
}
...
void CheckOther::checkRedundantAssignmentInSwitch()
{
    ...
    //   switch (var) { ...
    for (const Scope &switchScope : mCtx.symbolDB->scopeList) {
    ...
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (tok->link() && Token::Match(tok, "(|[|<"))
                ...
                labelName = tok->next();
            } else if (Token::Match(tok, "%name% (") && mCtx.project->library.isnoreturn(tok) && !Token::Match(tok->next()->astParent(), "?|:")) {
...
void CheckOther::duplicateBreakError(const Token *tok, bool inconclusive)
{
    reportError(tok, Severity::style, "duplicateBreak",
                ...
                "Consecutive return, break, continue, goto or throw statements are unnecessary. "
                "The second statement can never be executed, and so should be removed.", CWE561, inconclusive ? Certainty::inconclusive : Certainty::safe);
}

void CheckOther::unreachableCodeError(const Token *tok, bool inconclusive)
{
    reportError(tok, Severity::style, "unreachableCode",
                "Statements following return, break, continue, goto or throw will never be executed.", CWE561, inconclusive ? Certainty::inconclusive : Certainty::safe);
}
...
void CheckOther::checkVariableScope()
{
    ...
    for (const Variable* var : mCtx.symbolDB->variableList()) {
        ...
        if (mCtx.tokenizer->hasIfdef(var->nameToken(), var->scope()->bodyEnd))
            continue;
...
void CheckOther::variableScopeError(const Token *tok, const std::string &varname)
{
    reportError(tok,
                ...
                "    }\n"
                "}\n"
                "When you see this message it is always safe to reduce the variable scope 1 level.", CWE398, Certainty::safe);
}
...
void CheckOther::commaSeparatedReturnError(const Token *tok)
{
    reportError(tok,
                ...
                "macro is then used in a return statement, it is less likely such code is misunderstood.", CWE398, Certainty::safe);
}

//---------------------------------------------------------------------------
// Check for function parameters that should be passed by const reference
//---------------------------------------------------------------------------
static unsigned int estimateSize(const Type* type, const Context& ctx, unsigned int recursionDepth = 0)
{
    ...
    for (const Variable&var : type->classScope->varlist) {
        ...
        if (var.isPointer() || var.isReference())
            size = ctx.project->sizeof_pointer;
        else if (var.type() && var.type()->classScope)
            size = estimateSize(var.type(), ctx, recursionDepth+1);
        else if (var.valueType()->type == ValueType::Type::CONTAINER)
            size = 3 * ctx.project->sizeof_pointer; // Just guess
        else
            size = ctx.symbolDB->sizeOfType(var.typeStartToken());
    ...
    for (const Type::BaseInfo &baseInfo : type->derivedFrom) {
        if (baseInfo.type && baseInfo.type->classScope)
            cumulatedSize += estimateSize(baseInfo.type, ctx, recursionDepth+1);
    }
...
{
    if (!mCtx.project->severity.isEnabled(Severity::performance) || mCtx.tokenizer->isC())
        return;
    ...
    for (const Variable* var : mCtx.symbolDB->variableList()) {
        ...
        if (var->valueType()->type == ValueType::Type::CONTAINER) {
            ...
            if (!var->type()->classScope)
                inconclusive = true;
            else if (estimateSize(var->type(), mCtx) <= 2 * mCtx.project->sizeof_pointer)
                continue;
        } else
            continue;
...
void CheckOther::passedByValueError(const Token *tok, const std::string &parname, bool inconclusive)
{
    reportError(tok, Severity::performance, "passedByValue",
                ...
                "as a const reference which is usually faster and recommended in C++.", CWE398, inconclusive ? Certainty::inconclusive : Certainty::safe);
}
...
{
    if (!mCtx.project->severity.isEnabled(Severity::style) || mCtx.tokenizer->isC())
        return;
    ...
    for (const Variable *var : mCtx.symbolDB->variableList()) {
        ...
        if (isAliased(var))
            continue;
        if (isVariableChanged(var, mCtx.project, mCtx.tokenizer->isCPP()))
            continue;
...
void CheckOther::constVariableError(const Variable *var)
{
    ...
    reportError(tok, Severity::style, "const" + id, id + " '" + name + "' can be declared with const", CWE398, Certainty::safe);
}
...
{
    const bool warning = mCtx.project->severity.isEnabled(Severity::warning);
    const bool portability = mCtx.project->severity.isEnabled(Severity::portability);
    ...
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (Token::Match(tok, "%var% [")) {
                ...
                if (warning && tok->variable()->isArray() && astIsSignedChar(index) && index->getValueGE(0x80, mCtx.project))
                    signedCharArrayIndexError(tok);
                if (portability && astIsUnknownSignChar(index) && index->getValueGE(0x80, mCtx.project))
                    unknownSignCharArrayIndexError(tok);
            } else if (warning && Token::Match(tok, "[&|^]") && tok->isBinaryOp()) {
                bool warn = false;
                if (astIsSignedChar(tok->astOperand1())) {
                    const ValueFlow::Value *v1 = tok->astOperand1()->getValueLE(-1, mCtx.project);
                    const ValueFlow::Value *v2 = tok->astOperand2()->getMaxValue(false);
                    if (!v1)
                        v1 = tok->astOperand1()->getValueGE(0x80, mCtx.project);
                    if (v1 && !(tok->str() == "&" && v2 && v2->isKnown() && v2->intvalue >= 0 && v2->intvalue < 0x100))
                        warn = true;
                } else if (astIsSignedChar(tok->astOperand2())) {
                    const ValueFlow::Value *v1 = tok->astOperand2()->getValueLE(-1, mCtx.project);
                    const ValueFlow::Value *v2 = tok->astOperand1()->getMaxValue(false);
                    if (!v1)
                        v1 = tok->astOperand2()->getValueGE(0x80, mCtx.project);
                    if (v1 && !(tok->str() == "&" && v2 && v2->isKnown() && v2->intvalue >= 0 && v2->intvalue < 0x100))
                        warn = true;
                }
...
void CheckOther::signedCharArrayIndexError(const Token *tok)
{
    reportError(tok,
                ...
                "because of sign extension.", CWE128, Certainty::safe);
}
...
void CheckOther::unknownSignCharArrayIndexError(const Token *tok)
{
    reportError(tok,
                ...
                "treated depending on whether 'char' is signed or unsigned on target platform.", CWE758, Certainty::safe);
}
...
void CheckOther::charBitOpError(const Token *tok)
{
    reportError(tok,
                ...
                "    if (i & 0x8000)\n"
                "        printf(\"not expected\");\n"
                "The \"not expected\" will be printed on the screen.", CWE398, Certainty::safe);
}
...
void CheckOther::checkIncompleteStatement()
{
    ...
    for (const Token *tok = mCtx.tokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (isVoidStmt(tok))
            continue;
        if (mCtx.tokenizer->isCPP() && tok->str() == "&" && !(tok->astOperand1()->valueType() && tok->astOperand1()->valueType()->isIntegral()))
            // Possible archive
            continue;
        bool inconclusive = tok->isConstOp();
        if (mCtx.project->certainty.isEnabled(Certainty::inconclusive) || !inconclusive)
...
void CheckOther::constStatementError(const Token *tok, const std::string &type, bool inconclusive)
{
    ...
    if (Token::simpleMatch(tok, "=="))
        ...
        return; // Strange!
    reportError(tok, Severity::warning, "constStatement", msg, CWE398, inconclusive ? Certainty::inconclusive : Certainty::safe);
}
...
{
    if (!tok && !value) {
        reportError(tok, Severity::error, "zerodiv", "Division by zero.", CWE369, Certainty::safe);
        reportError(tok, Severity::error, "zerodivcond", ValueFlow::eitherTheConditionIsRedundant(nullptr) + " or there is division by zero.", CWE369, Certainty::safe);
        return;
    }
    ...
    reportError(errorPath,
                ...
                errmsg.str(), CWE369, value->isInconclusive() ? Certainty::inconclusive : Certainty::safe);
}
...
void CheckOther::nanInArithmeticExpressionError(const Token *tok)
{
    reportError(tok, Severity::style, "nanInArithmeticExpression",
                ...
                "Using NaN/Inf in a computation. "
                "Although nothing bad really happens, it is suspicious.", CWE369, Certainty::safe);
}
...
void CheckOther::misusedScopeObjectError(const Token *tok, const std::string& varname)
{
    reportError(tok, Severity::style,
                "unusedScopedObject",
                "$symbol:" + varname + "\n"
                "Instance of '$symbol' object is destroyed immediately.", CWE563, Certainty::safe);
}
    ...
    for (const Scope & scope : mCtx.symbolDB->scopeList) {
        ...
        if (Token::simpleMatch(scope.bodyEnd, "} else {")) {
            ...
            if (branchTop1->str() != branchTop2->str())
                continue;
            if (isSameExpression(mCtx.tokenizer->isCPP(), false, branchTop1->astOperand1(), branchTop2->astOperand1(), mCtx.project->library, true, true, &errorPath) &&
                isSameExpression(mCtx.tokenizer->isCPP(), false, branchTop1->astOperand2(), branchTop2->astOperand2(), mCtx.project->library, true, true, &errorPath))
                duplicateBranchError(scope.classDef, scope.bodyEnd->next(), errorPath);
        }
    }
}
...
void CheckOther::checkInvalidFree()
{
    ...
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            else if (Token::Match(tok, "%name% (") && !mCtx.project->library.isFunctionConst(tok->str(), true)) {
...
void CheckOther::invalidFreeError(const Token *tok, const std::string &allocation, bool inconclusive)
{
    ...
    reportError(tok, Severity::error, "invalidFree", "Mismatching address is " + deallocated + ". The address you get from " + alloc + " must be " + deallocated + " without offset.", CWE(0U), inconclusive ? Certainty::inconclusive : Certainty::safe);
}
...
{
    const bool styleEnabled = mCtx.project->severity.isEnabled(Severity::style);
    const bool warningEnabled = mCtx.project->severity.isEnabled(Severity::warning);
    if (!styleEnabled && !warningEnabled)
        return;

    // Parse all executing scopes..
    std::list<const Function*> constFunctions;
    getConstFunctions(mCtx.symbolDB, constFunctions);
    ...
    for (const Scope *scope : mCtx.symbolDB->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (tok->str() == "=" && Token::Match(tok->astOperand1(), "%var%")) {
                ...
                if (Token::Match(endStatement, "%var% %assign%")) {
                    ...
                    if (var1 && var2 &&
                        ...
                        isSameExpression(mCtx.tokenizer->isCPP(), true, tok->next(), nextAssign->next(), mCtx.project->library, true, false) &&
                        isSameExpression(mCtx.tokenizer->isCPP(), true, tok->astOperand2(), nextAssign->astOperand2(), mCtx.project->library, true, false) &&
                        ...
                        else if (mCtx.project->certainty.isEnabled(Certainty::inconclusive))
            ...
            if (tok->isOp() && tok->astOperand1() && !Token::Match(tok, "+|*|<<|>>|+=|*=|<<=|>>=")) {
                ...
                if (isSameExpression(mCtx.tokenizer->isCPP(),
                                     true,
                                     tok->astOperand1(),
                                     tok->astOperand2(),
                                     mCtx.project->library,
                                     true,
                                     followVar,
                                     &errorPath)) {
                    if (isWithoutSideEffects(mCtx.tokenizer->isCPP(), tok->astOperand1())) {
                        ...
                        if (assignment && warningEnabled)
                           ...
                           isSameExpression(mCtx.tokenizer->isCPP(),
                                            false,
                                            tok->astOperand1(),
                                            ...
                                            true,
                                            false)) {
                    if (warningEnabled && isWithoutSideEffects(mCtx.tokenizer->isCPP(), tok->astOperand1())) {
                        selfAssignmentError(tok, tok->astOperand1()->expressionString());
                    }
                } else if (styleEnabled &&
                           isOppositeExpression(mCtx.tokenizer->isCPP(),
                                                tok->astOperand1(),
                                                tok->astOperand2(),
                                                mCtx.project->library,
                                                false,
                                                ...
                                                &errorPath) &&
                           !Token::Match(tok, "=|-|-=|/|/=") &&
                           isWithoutSideEffects(mCtx.tokenizer->isCPP(), tok->astOperand1())) {
                    ...
                    if (styleEnabled && tok->astOperand2() && tok->str() == tok->astOperand1()->str() &&
                        isSameExpression(mCtx.tokenizer->isCPP(),
                                         ...
                                         followVar,
                                         &errorPath) &&
                        isWithoutSideEffects(mCtx.tokenizer->isCPP(), tok->astOperand2()))
                        duplicateExpressionError(tok->astOperand2(), tok->astOperand1()->astOperand2(), tok, errorPath);
                    else if (tok->astOperand2() && isConstExpression(tok->astOperand1(), mCtx.project->library, true, mCtx.tokenizer->isCPP())) {
                        const Token *ast1 = tok->astOperand1();
                        while (ast1 && tok->str() == ast1->str()) {
                            if (isSameExpression(mCtx.tokenizer->isCPP(), true, ast1->astOperand1(), tok->astOperand2(), mCtx.project->library, true, true, &errorPath) &&
                                isWithoutSideEffects(mCtx.tokenizer->isCPP(), ast1->astOperand1()) &&
                                isWithoutSideEffects(mCtx.tokenizer->isCPP(), ast1->astOperand2()))
                ...
                else if (isSameExpression(mCtx.tokenizer->isCPP(), true, tok->astOperand1(), tok->astOperand2(), mCtx.project->library, false, true, &errorPath))
...
void CheckOther::oppositeExpressionError(const Token *opTok, ErrorPath errors)
{
    ...
    reportError(errors, Severity::style, "oppositeExpression", "Opposite expression on both sides of \'" + op + "\'.\n"
                ...
                "determine if it is correct.", CWE398, Certainty::safe);
}
...
void CheckOther::duplicateAssignExpressionError(const Token *tok1, const Token *tok2, bool inconclusive)
{
    ...
    reportError(toks, Severity::style, "duplicateAssignExpression",
                ...
                "determine if it is correct.", CWE398, inconclusive ? Certainty::inconclusive : Certainty::safe);
}
...
void CheckOther::selfAssignmentError(const Token *tok, const std::string &varname)
{
    reportError(tok, Severity::warning,
                "selfAssignment",
                "$symbol:" + varname + "\n"
                "Redundant assignment of '$symbol' to itself.", CWE398, Certainty::safe);
}
...
void CheckOther::checkComparisonFunctionIsAlwaysTrueOrFalseError(const Token* tok, const std::string &functionName, const std::string &varName, const bool result)
{
    ...
    reportError(tok, Severity::warning, "comparisonFunctionIsAlwaysTrueOrFalse",
                ...
                "for both parameters leads to a statement which is always " + strResult + ".", cweResult, Certainty::safe);
}
...
void CheckOther::unsignedLessThanZeroError(const Token *tok, const ValueFlow::Value * v, const std::string &varname)
{
    reportError(getErrorPath(tok, v, "Unsigned less than zero"), Severity::style, "unsignedLessThanZero",
                ...
                "is either pointless or an error to check if it is.", CWE570, Certainty::safe);
}
...
void CheckOther::pointerLessThanZeroError(const Token *tok, const ValueFlow::Value *v)
{
    reportError(getErrorPath(tok, v, "Pointer less than zero"), Severity::style, "pointerLessThanZero",
                "A pointer can not be negative so it is either pointless or an error to check if it is.", CWE570, Certainty::safe);
}
...
void CheckOther::unsignedPositiveError(const Token *tok, const ValueFlow::Value * v, const std::string &varname)
{
    reportError(getErrorPath(tok, v, "Unsigned positive"), Severity::style, "unsignedPositive",
                "$symbol:" + varname + "\n"
                "Unsigned expression '$symbol' can't be negative so it is unnecessary to test it.", CWE570, Certainty::safe);
}
...
void CheckOther::pointerPositiveError(const Token *tok, const ValueFlow::Value * v)
{
    reportError(getErrorPath(tok, v, "Pointer positive"), Severity::style, "pointerPositive",
                "A pointer can not be negative so it is either pointless or an error to check if it is not.", CWE570, Certainty::safe);
}
...
{
    if (!mCtx.project->severity.isEnabled(Severity::performance) || mCtx.tokenizer->isC() || !mCtx.project->certainty.isEnabled(Certainty::inconclusive))
        return;
...
static bool isNegative(const Token *tok, const Project* project)
{
    return tok->valueType() && tok->valueType()->sign == ValueType::SIGNED && tok->getValueLE(-1LL, project);
}
...
void CheckOther::checkNegativeBitwiseShift()
{
    ...
    for (const Token* tok = mCtx.tokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (portability && isNegative(tok->astOperand1(), mCtx.project))
            negativeBitwiseShiftError(tok, 1);
        else if (isNegative(tok->astOperand2(), mCtx.project))
...
void CheckOther::negativeBitwiseShiftError(const Token *tok, int op)
{
    if (op == 1)
        ...
        // a portability issue
        reportError(tok, Severity::portability, "shiftNegativeLHS", "Shifting a negative value is technically undefined behaviour", CWE758, Certainty::safe);
    else // RHS
        reportError(tok, Severity::error, "shiftNegative", "Shifting by a negative value is undefined behaviour", CWE758, Certainty::safe);
}
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (Token::Match(tok, "memset|memcpy|memmove (") && Token::Match(tok->linkAt(1)->tokAt(-2), ", %num% )")) {
                ...
                if (MathLib::toLongNumber(tok->linkAt(1)->strAt(-1)) == var->dimension(0)) {
                    unsigned int size = mCtx.tokenizer->sizeOfType(var->typeStartToken());
                    if (size == 0 && var->valueType()->pointer)
                        size = mCtx.project->sizeof_pointer;
                    else if (size == 0 && var->type())
                        size = estimateSize(var->type(), mCtx);
...
void CheckOther::redundantPointerOpError(const Token* tok, const std::string &varname, bool inconclusive)
{
    reportError(tok, Severity::style, "redundantPointerOp",
                "$symbol:" + varname + "\n"
                "Redundant pointer operation on '$symbol' - it's already a pointer.", CWE398, inconclusive ? Certainty::inconclusive : Certainty::safe);
}

void CheckOther::checkInterlockedDecrement()
{
    if (!mCtx.project->isWindowsPlatform()) {
        return;
    }
...
void CheckOther::raceAfterInterlockedDecrementError(const Token* tok)
{
    reportError(tok, Severity::error, "raceAfterInterlockedDecrement",
                "Race condition: non-interlocked access after InterlockedDecrement(). Use InterlockedDecrement() return value instead.", CWE362, Certainty::safe);
}

void CheckOther::checkUnusedLabel()
{
    if (!mCtx.project->severity.isEnabled(Severity::style) && !mCtx.project->severity.isEnabled(Severity::warning))
        return;

    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        const bool hasIfdef = mCtx.tokenizer->hasIfdef(scope->bodyStart, scope->bodyEnd);
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
...
{
    if (tok && !mCtx.project->severity.isEnabled(inSwitch ? Severity::warning : Severity::style))
        return;
...
void CheckOther::checkEvaluationOrder()
{
    ...
    if (mCtx.tokenizer->isCPP() && mCtx.project->standards.cpp >= Standards::CPP11)
        return;
        ...
        for (const Token* tok = functionScope->bodyStart; tok != functionScope->bodyEnd; tok = tok->next()) {
            ...
            for (const Token *tok2 = tok;; tok2 = tok2->astParent()) {
                ...
                if (tok2 == tok &&
                    tok->str() == "=" &&
                    parent->str() == "=" &&
                    isSameExpression(mCtx.tokenizer->isCPP(), false, tok->astOperand1(), parent->astOperand1(), mCtx.project->library, true, false)) {
                    if (mCtx.project->severity.isEnabled(Severity::warning) &&
                        isSameExpression(mCtx.tokenizer->isCPP(), true, tok->astOperand1(), parent->astOperand1(), mCtx.project->library, true, false))
                ...
                [&](const Token *tok3) {
                    ...
                    if (isSameExpression(mCtx.tokenizer->isCPP(), false, tok->astOperand1(), tok3, mCtx.project->library, true, false))
                        foundError = true;
...
void CheckOther::unknownEvaluationOrder(const Token* tok)
{
    reportError(tok, Severity::error, "unknownEvaluationOrder",
                "Expression '" + (tok ? tok->expressionString() : std::string("x = x++;")) + "' depends on order of evaluation of side effects", CWE768, Certainty::safe);
}

void CheckOther::checkAccessOfMovedVariable()
{
    if (!mCtx.tokenizer->isCPP() || mCtx.project->standards.cpp < Standards::CPP11 || !mCtx.project->severity.isEnabled(Severity::warning))
        return;
    CheckUninitVar checkUninitVar(mCtx);
    const bool reportInconclusive = mCtx.project->certainty.isEnabled(Certainty::inconclusive);
    for (const Scope * scope : mCtx.symbolDB->functionScopes) {
        ...
        for (const Token* tok = scopeStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (tok->strAt(1) == ".") {
                ...
                const bool isVarChanged = isVariableChangedByFunctionCall(tok, 0, mCtx.project, &inconclusive);
...
{
    if (!tok) {
        reportError(tok, Severity::warning, "accessMoved", "Access of moved variable 'v'.", CWE672, Certainty::safe);
        reportError(tok, Severity::warning, "accessForwarded", "Access of forwarded variable 'v'.", CWE672, Certainty::safe);
        return;
    }
    ...
    reportError(errorPath, Severity::warning, errorId, errmsg, CWE672, inconclusive ? Certainty::inconclusive : Certainty::safe);
}
...
{
    const bool style = mCtx.project->severity.isEnabled(Severity::style);
                                       ...
                                       const std::vector<const Token *> & definitions)
{
    ...
    msg += "'";
    reportError(tokens, Severity::warning, "funcArgOrderDifferent", msg, CWE683, Certainty::safe);
}
...
void CheckOther::shadowError(const Token *var, const Token *shadowed, std::string type)
{
    ...
    reportError(errorPath, Severity::style, id.c_str(), message, CWE398, Certainty::safe);
}
...
void CheckOther::knownArgumentError(const Token *tok, const Token *ftok, const ValueFlow::Value *value)
{
    ...
    const ErrorPath errorPath = getErrorPath(tok, value, errmsg);
    reportError(errorPath, Severity::style, "knownArgument", errmsg, CWE570, Certainty::safe);
}
