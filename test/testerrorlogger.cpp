...
private:
    ...
    void ErrorMessageConstruct() const {
        ...
        ErrorMessage msg(locs, emptyString, Severity::error, "Programming error.", "errorId", Certainty::safe);
    ...
    void ToXmlV2Encoding() const {
        {
            ...
            ErrorMessage msg(locs, emptyString, Severity::error, "Programming error.\nComparing \"\203\" with \"\003\"", "errorId", Certainty::safe);
    ...
    void InconclusiveXml() const {
        ...
        ASSERT_EQUALS("        <error id=\"errorId\" severity=\"error\" msg=\"Programming error\" verbose=\"Programming error\" certainty=\"inconclusive\">\n"
    ...
    void SerializeSanitize() const {
        std::list<ErrorMessage::FileLocation> locs;
        ErrorMessage msg(locs, emptyString, Severity::error, std::string("Illegal character in \"foo\001bar\""), "errorId", Certainty::safe);
